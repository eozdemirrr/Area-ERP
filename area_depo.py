...
def veritabanini_kaydet(db):
    try:
        ...
        if cevap.status_code != 200:
            st.error(f"❌ Veritabanı Hatası: {cevap.status_code} - {cevap.text}")
            return False
        return True
    except:
...
# --- KRİTİK: YASAKLI KARAKTER TEMİZLEYİCİ ---
def isim_temizle(metin):
    # Firebase'in yasakladığı karakterleri tire (-) ile değiştirelim
    yasakli = [".", "$", "#", "[", "]", "/"]
    for char in yasakli:
...
if not st.session_state["logged_in"]:
    ...
    st.markdown("<h2 style='text-align: center;'>AREA İKLİMLENDİRME ERP GİRİŞİ</h2>", unsafe_allow_html=True)
        ...
        with st.form("login_form"):
            ...
            if st.form_submit_button("Giriş Yap", use_container_width=True):
                ...
                if k_adi in kullanicilar_db and kullanicilar_db[k_adi]["sifre"] == sifre:
                    st.session_state["logged_in"], st.session_state["kullanici"] = True, k_adi
...
st.sidebar.success(f"👋 {st.session_state['isim']}")
st.sidebar.markdown("---")
sayfalar = []
...
if secilen_sayfa == "📦 Depo Yönetim Ekranı":
    ...
    if not db["stok"]: st.warning("⚠️ Stok yok.")
    else:
        with st.form("depo_cikis_formu", clear_on_submit=True):
            urun = st.selectbox("Ürün:", sorted(list(db["stok"].keys())))
            ...
            notlar = c4.text_input("Ek Notlar:")
            if st.form_submit_button("🚚 ÇIKIŞ YAP"):
                if firma == "": st.error("Firma adı gerekli!")
                else:
                    db["stok"][urun] -= adet
    ...
    if not bekleyenler: st.success("Bekleyen işlem yok.")
        ...
        for islem in bekleyenler:
            with st.expander(f"🔴 {islem['firma']} | {islem['tarih_cikis']}", expanded=True):
                ...
                with st.form(f"fiyat_f_{islem['id']}"):
                    f = st.number_input("Satış Bedeli (₺):", min_value=0.0, step=500.0)
                    if st.form_submit_button("💰 Onayla"):
                        islem["fiyat"], islem["durum"] = f, "Fatura Bekliyor"
...
# --- 4. STOK ENVANTERİ ---
    ...
    if st.session_state["rol"] in ["Yönetici", "Depo"]:
        ...
        with st.form("stok_ekle_f", clear_on_submit=True):
            c1, c2, c3, c4 = st.columns(4)
            marka = c1.text_input("Marka:").upper()
            seri = c2.text_input("Seri:").upper()
            model = isim_temizle(c3.text_input("Model Kodu:").upper()) # TEMİZLİK BURADA
            ...
            if st.form_submit_button("📥 Envantere Ekle"):
                ...
                if veritabanini_kaydet(db): st.success("✅ Ürün eklendi!"); st.rerun()

    st.subheader("📦 Mevcut Depo Stokları")
    if st.session_state["rol"] == "Servis": st.info("ℹ️ Servis Yetkisi: Sadece görüntüleme.")
    ...
    if db.get("stok"):
        ...
        for k in KATEGORILER + ["Diğer"]:
            ...
            if urunler:
                ...
                html_t += f"<h3>{k}</h3><table border='1' style='width:100%;'><tr><th>Kategori</th><th>Marka</th><th>Seri</th><th>Model</th><th>Adet</th></tr>"
        ...
        if st_var:
            ...
            h_s = f"<html><body style='font-family:Arial;'><h1>AREA STOK ({t})</h1>{html_t}<script>window.onload=function(){{window.print();}}</script></body></html>"
            st.download_button("📥 Raporu İndir/Yazdır", data=h_s, file_name=f"Stok_{t}.html", mime="text/html", use_container_width=True)
    ...
    with t2:
        if db["stok"]:
            ...
            if sil != "Seçiniz..." and st.button("🚨 SİL", type="primary"):
                ...
                if veritabanini_kaydet(db): st.success("Ürün silindi!"); st.rerun()
    with t3:
